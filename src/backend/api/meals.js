...
router.get("/", async (request, response) => {
  try {
    if (request.query.maxPrice) {
      ...
      const mealsWithMaxPrice = await knex("meals").where(
        "price",
        "<",
        maxPrice
      );
      ...
      const givenTitle = request.query.title;
      const mealsWithTittle = await knex("meals").where(
        "tittle",
        "like",
        "%" + givenTitle + "%"
      );
      ...
      const mealsWithLimit = await knex("meals").limit(givenLimit);

      response.json(mealsWithLimit);
    } else if (request.query.availableReservations) {
      const mealsWithReservations = await knex("meals").where(
        "max_reservations",
        ">",
        0
      );
      ...
      const mealsCreatedAfterDate = await knex("meals").where(
        "created_date",
        ">",
        givenDate
      );
